...
describe('action queue', () => {
  it('runs a queued chain sequentially, including a failed attempt, with no idle tick between actions', async () => {
    ...
    engine.registerActionType({
      type: 'chop_wood',
      durationTicks: 60,
      resolve: () => ({ success: true, message: 'Chopped a load of wood.', data: { item: 'timber', qty: 1 } }),
    });
    engine.registerActionType({
      type: 'botched_smelt',
      durationTicks: 40,
    ...
    // exclusively).
    expect(history[0].startedAtTick).toBe(1);
    expect(history[0].endsAtTick).toBe(61);
    expect(history[1].startedAtTick).toBe(61);
    expect(history[1].endsAtTick).toBe(101);
    expect(history[2].startedAtTick).toBe(101);
    expect(history[2].endsAtTick).toBe(161);

    expect(history[1].outcome?.success).toBe(false);
    expect(history[0].outcome?.data).toEqual({ item: 'timber', qty: 1 });
  ...
  it('marks an in-progress action interrupted with proportional partial progress', async () => {
    ...
    const [action] = engine.getActorActions('hauler-1');
  ...
  it('throws when queueing an unregistered action type', async () => {
    ...
    expect(() => engine.queueAction('villager-2', 'nonexistent_action')).toThrow(
      /Unknown action type/,
    );
