...
describe('item provenance', () => {
  it('records a full life — produced, transferred, then consumed — as a queryable chain', async () => {
    ...
    expect(chain[0].tick).toBe(5)
    expect(chain[0].toContainerId).toBe('riverside-mill-storage')
    expect(chain[1].tick).toBe(8)
    expect(chain[1].fromContainerId).toBe('riverside-mill-storage')
    expect(chain[1].toContainerId).toBe('market-stall-2')
    expect(chain[2].tick).toBe(18)
    expect(chain[2].actorId).toBe('villager-1')
