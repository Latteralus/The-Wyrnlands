...
export function getConservationCounters(db: Database): ConservationCounters {
  ...
  // world_meta always has exactly its one id=1 row from Engine.bootstrap
  // onward; a missing row means the engine was never bootstrapped.
  if (!row) throw new Error('world_meta row missing — was Engine.bootstrap() called?');
  return {
