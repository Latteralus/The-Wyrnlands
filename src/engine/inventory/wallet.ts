...
export function getBalance(db: Database, ownerId: string): number {
  const result = db.exec('SELECT balance FROM wallets WHERE owner_id = ?', [ownerId])
  if (result.length === 0) return 0
  return Number(result[0].values[0][0])
}

export function sumWalletBalances(db: Database): number {
  const result = db.exec('SELECT COALESCE(SUM(balance), 0) FROM wallets')
  return Number(result[0].values[0][0])
}
...
// coin should grow.
export function faucetCoin(db: Database, bus: EventBus, ownerId: string, amount: number, tick: number, note?: string): void {
...
export function sinkCoin(db: Database, bus: EventBus, ownerId: string, amount: number, tick: number, note?: string): void {
  if (amount <= 0) throw new Error(`sinkCoin amount must be positive, got ${amount}`)
  const balance = getBalance(db, ownerId)
  if (balance < amount) throw new Error(`Insufficient balance: ${ownerId} has ${balance}, tried to sink ${amount}`)
